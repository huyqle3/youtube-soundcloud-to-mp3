...
$(document).ready(function() {
    ...
    var vid = '';
    $.ajax({
        url: 'https://www.youtube2mp3.cc/p.php',
        dataType: 'jsonp',
        data: {
    ...
    function Error() {
        alert('A error occured. Please try to convert another video.');
    }

    function VideoSyntax(v) {
        if (new RegExp(/youtube.com/).test(v) && (vid = new RegExp(/v\=[a-zA-Z0-9\-\_]{11}/).exec(v))) {
            vid = vid.toString().substr(2);
        } else if (new RegExp(/youtu.be/).test(v) && (vid = new RegExp(/\/[a-zA-Z0-9\-\_]{11}/).exec(v))) {
            vid = vid.toString().substr(1);
        }
